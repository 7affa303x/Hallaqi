      ...
      <div className="min-h-screen flex flex-col items-center justify-center px-6" style={{ backgroundColor: themeConfig.colors.background }}>
        <div className="text-center max-w-xs">
          ...
          <p className="text-sm mb-6 leading-relaxed" style={{ color: themeConfig.colors.textMuted }}>
            سجل الدخول للوصول لحجوزاتك، المحادثات، والمزيد من الميزات
          </p>
          ...
          <div className="space-y-2">
            ...
            <button
              ...
              className="w-full h-12 rounded-xl text-sm font-bold border transition-all flex items-center justify-center gap-2"
          ...
          <p className="text-[10px] mt-4" style={{ color: themeConfig.colors.textMuted }}>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            ...
            <div className="flex items-center gap-2 mt-1.5">
              ...
              {userRole === 'admin' && (
                <span className="text-[10px] px-2 py-0.5 rounded-full bg-red-500/30 text-white font-medium">
                  مشرف
                </span>
              )}
            </div>
...
function IDVerification({ onBack }: { onBack: () => void }) {
  const { themeConfig } = useApp();
  // Use auth hook to check real verification status
    ...
    <div className="pb-20">
      ...
      <div className="px-4 mt-4 space-y-2">
        {providers.map(provider => {
          const account = currentUser?.linkedAccounts?.find((a: LinkedAccount) => a.provider === provider.key);
          return (
            <div
              key={provider.key}
    ...
    <div className="pb-20">
      ...
      <div className="px-4 mt-4">
        <div className="grid grid-cols-2 gap-2">
          {badges.map((badge: { id: string; name: string; description: string; color: string }) => (
            <div
              key={badge.id}
...
function StatsPage({ onBack }: { onBack: () => void }) {
  const { themeConfig, currentUser } = useApp();
  const { appUser } = useAuth();
  const stats = (appUser?.stats as UserStats) || (currentUser as unknown as { stats?: UserStats })?.stats || { totalBookings: 0, totalSpent: 0, streakDays: 0, points: 0, rank: 'جديد' };
  ...
  return (
    <div className="pb-20">
      ...
      <div className="px-4 mt-4 space-y-3">
        ...
        <div className="grid grid-cols-2 gap-2">
          {[
            { label: 'إجمالي الحجوزات', value: (stats.totalBookings || 0).toString(), icon: Calendar, color: '#3B82F6' },
            { label: 'إجمالي الإنفاق', value: `${stats.totalSpent || 0} دج`, icon: CreditCard, color: '#22C55E' },
            { label: 'الأيام المتتالية', value: (stats.streakDays || 0).toString(), icon: Zap, color: '#F59E0B' },
            { label: 'النقاط', value: (stats.points || 0).toString(), icon: Star, color: '#8B5CF6' },
          ].map((stat, i) => (
            <div
              key={i}
